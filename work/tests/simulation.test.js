...
describe("balance simulations", () => {
  it("Strategy 1 clears Level 1 using Stoneguard Post upgrades (Watchfire & Oath Stones)", () => {
    ...
    const actions = {
      0: (g) => {
        expectOk(g.placeTower("punch", 5, 1)); // cost 28 (hits path at [6,2])
        expectOk(g.placeTower("punch", 2, 4)); // cost 28 (hits path at [3,3])
      },
      1: (g) => {
        expectOk(g.upgradeTower(0, 1)); // Watchfire: cost 35
        expectOk(g.upgradeTower(1, 0)); // Oath Stones: cost 35
      }
    };
    ...
    expect(game.towers.map((tower) => tower.upgradeHistory[0]?.id)).toEqual(["watchfire", "oath-stones"]);
  });
  ...
  it("Strategy 2 clears Level 1 using Arcane Spire upgrades (Blue Spark chain lightning)", () => {
    ...
    const actions = {
      0: (g) => {
        expectOk(g.placeTower("radio", 5, 1)); // cost 42
        expectOk(g.upgradeTower(0, 1)); // Blue Spark: cost 28 (chain bounces)
      },
      1: (g) => {
        expectOk(g.placeTower("radio", 2, 4)); // cost 42
      }
    };
  ...
  it("Strategy 3 clears Level 1 using Frost Obelisk + Bounty Ballista coverage", () => {
    const game = new GameEngine(1); // Briar Glen: 86 gold

    const actions = {
      0: (g) => {
        expectOk(g.placeTower("punch", 10, 3)); // Stoneguard: cost 28 (hits path around [9,3]/[11,3])
      },
      1: (g) => {
        expectOk(g.sellTower(0)); // returns +19 gold
        expectOk(g.placeTower("freezer", 5, 1)); // cost 63 (slows path at [6,2])
        expectOk(g.placeTower("tax", 2, 4)); // cost 49
      }
    };
    ...
    expect(game.status).toBe("won");
    expect(game.lives).toBeGreaterThan(0);
    expect(game.towers.map((tower) => tower.type)).toEqual(["freezer", "tax"]);
  });

  it("proves a poorly covered single-tower Level 2 build leaks elites", () => {
    const game = new GameEngine(2); // Mossgate Ford: 112 gold, contains heavy Stonebacks

    const actions = {
      0: (g) => {
        expectOk(g.placeTower("punch", 1, 0)); // Out of range of path
      }
    };
  ...
  it("proves Shield Breaker counters shielded acolytes efficiently", () => {
    ...
    expectOk(game.placeTower("radio", 1, 0)); // Reaches path [0,2] at distance 2.236
    ...
    expectOk(game.upgradeTower(0, 0));
